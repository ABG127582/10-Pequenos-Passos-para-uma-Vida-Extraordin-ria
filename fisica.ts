...
const renderTasks = () => {
    ...
    const tasksForToday = allTasks.filter(task => task.dueDate === todayStr && task.category === 'Física');

    elements.tasksList.innerHTML = '';

    if (tasksForToday.length === 0) {
        elements.tasksList.innerHTML = '<li class="empty-list-placeholder">Nenhuma tarefa de Saúde Física agendada para hoje.</li>';
        return;
    }
...
const handleTaskAction = async (e: Event) => {
    ...
    if (target.closest('.edit-btn') || target.closest('.item-text-wrapper')) {
        ...
        if (newCompletedStatus && !wasCompleted) {
            ...
            const categoryTasks = getTasks().filter(t => t.category === 'Física' && t.dueDate === task.dueDate);
            ...
            if (allCategoryTasksCompleted) {
                ...
                window.showToast('Parabéns! Você completou todas as tarefas de Saúde Física e ganhou uma medalha!', 'success');
            }
        }
    }
};
...
export function setup() {
    ...
    elements.tasksList = page.querySelector('#fisica-tasks-list');
    elements.addTaskBtn = page.querySelector('#add-fisica-task-btn');
    ...
    elements.addTaskBtn?.addEventListener('click', () => {
        openTaskModal(undefined, { category: 'Física', title: '' });
    });
    ...
    document.body.addEventListener('datachanged:tasks', () => {
        if (window.location.hash === '#fisica') {
             renderTasks();
        }
    });
}
