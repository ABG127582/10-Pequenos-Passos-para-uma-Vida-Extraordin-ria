...
const renderTasks = () => {
    ...
    const tasksForToday = allTasks.filter(task => task.dueDate === todayStr && task.category === 'Social');

    elements.tasksList.innerHTML = '';

    if (tasksForToday.length === 0) {
        elements.tasksList.innerHTML = '<li class="empty-list-placeholder">Nenhuma tarefa de Saúde Social agendada para hoje.</li>';
        return;
    }
...
const handleTaskAction = async (e: Event) => {
    ...
    if (target.closest('.edit-btn') || target.closest('.item-text-wrapper')) {
        ...
        if (newCompletedStatus && !wasCompleted) {
            ...
            const categoryTasks = getTasks().filter(t => t.category === 'Social' && t.dueDate === task.dueDate);
            ...
            if (allCategoryTasksCompleted) {
                ...
                window.showToast('Parabéns! Você completou todas as tarefas de Saúde Social e ganhou uma medalha!', 'success');
            }
        }
    }
};
...
export function setup() {
    ...
    elements.tasksList = page.querySelector('#social-tasks-list');
    elements.addTaskBtn = page.querySelector('#add-social-task-btn');
    elements.reflectionForm = page.querySelector('.reflection-form');
    
    elements.addTaskBtn?.addEventListener('click', () => {
        openTaskModal(undefined, { category: 'Social', title: '' });
    });
